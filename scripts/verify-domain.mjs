...
if (validateSquad(squad).length) throw new Error('demo squad must be legal')
const clubViolation = squad.map((player, index) => index < 4 ? { ...player, club: 'ARS' } : player)
if (!validateSquad(clubViolation).some(issue => issue.rule === 'Club limit')) throw new Error('club limit is not enforced')
