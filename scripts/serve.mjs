...
async function reprocessRemoteSignal(db, signal, { includeVerified = false } = {}) {
  ...
  const roleEvidence=/\b(?:first[ -]?choice|regular starter|starting (?:xi|line[- ]?up|striker|keeper|goalkeeper|centre-back)|number one|no real competition|nailed(?: on| down)?|guaranteed (?:to )?start|undisputed starter|main man|first on the team sheet|clear first choice|preferred starter|lock in the (?:xi|lineup)|lead the (?:line|attack)|spearhead the attack|assured of (?:his|her|their) place|set to start|likely to start|expects? (?:him|her|them) to start|expected to start|will start|won't start|not expected to|regular starts?|rotation|rotat(?:e|ion)|bench risk|rotation risk|minutes (?:risk|concern|managed)|split minutes|share minutes|one of two|compete? for minutes|competition for minutes|may not start|unavailable|ruled out|will miss|out for|sidelined|out injured|back[ -]?up|second[ -]?choice|third[ -]?choice|reserve (?:keeper|goalkeeper))\b/i.test(text)
  const conditionalOpinion=/\b(?:would pick if|if .*likely to start|should be avoided|could kill .*pick)\b/i.test(text)
  const unavailableEvidence=/\b(?:ruled out|unavailable|will miss|going to miss|set to miss|out for (?:weeks|months|the season)|miss the start of the season|sidelined|out injured|injury layoff|suffered an? injury|knee injury|hamstring (?:injury|strain|tear)|torn (?:acl|cruciate|hamstring|meniscus|ligament)|broken (?:leg|foot|ankle|arm)|underwent surgery|surgery|long[ -]term (?:injury|absentee)|suspended|red card ban|banned for \d+ (?:games|matches)|failed (?:a )?fitness test|not fit to feature|not in contention|out of contention)\b/i.test(text)
...
For claims about real-world availability or minutes, always include suggestedInterpretation: {role: FIRST_CHOICE|ROTATION_LOW|ROTATION_MEDIUM|ROTATION_HIGH|BACKUP|OUT, confidence: 0 to 1, rationale: string}. This is a proposed translation for the FPL model, not a claim that the creator supplied a number. Use it whenever the wording implies a starting role or availability: "nailed", "expected first choice", "guaranteed to start", "starts every game" means FIRST_CHOICE; "not nailed", "no fixed number one", "competition for minutes", "all positions are up for grabs" means ROTATION_MEDIUM; "bench risk", "may not get regular starts", "one of two" means ROTATION_HIGH; "backup", "second choice", "reserve keeper" means BACKUP; "ruled out", "injured", "sidelined", "will miss", "torn ACL" means OUT. Treat a conditional transfer, a possible future signing, or a player's own FPL selection as context only unless the speaker also makes a clear current role claim. Never add an interpretation to FPL_SELECTION, value opinions, or vague player mentions.
