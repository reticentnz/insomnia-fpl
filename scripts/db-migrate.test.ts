...
describe('canonical database migrations', () => {
  it('applies the initial migration and makes the second run a no-op', async () => {
    ...
    const migrations = ['001_initial_rebuild', '002_app_state_and_manager_totals', '003_remove_app_user_api_key', '004_recommendation_cache_index', '005_draft_and_season_mode', '006_signal_interpretations', '007_reclassify_creator_bench_context', '008_league_differential', '009_signal_source_date']
    ...
    expect(Number(tables.count)).toBe(31)
    db.close()
  })
