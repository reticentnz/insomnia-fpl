...
describe('canonical HTTP API smoke', () => {
  it('serves catalog, compatibility data, markets, and auditable signal writes from a fresh migrated fixture database', async () => {
    ...
    const server = spawn(process.execPath, ['--experimental-strip-types', 'scripts/serve.mjs'], { cwd: process.cwd(), env: { ...process.env, PORT: String(port), HOST: '127.0.0.1', DATABASE_URL: `file:${databasePath}`, FPL_SEASON: '2026/27', FPL_INGEST_INTERVAL_HOURS: '876000', ADMIN_TOKEN: 'fixture-admin-token', SIGNAL_INGEST_TOKEN: 'fixture-signal-token', ODDS_API_KEY: ' ', FPL_CATALOG_CACHE_FILE: path.join(directory, 'catalog-cache.json'), SIGNAL_CONFIG_FILE: path.join(directory, 'signal-config.json'), AI_SETTINGS_FILE: path.join(directory, 'ai-settings.json') }, stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    expect(admin.operations).toHaveLength(5)
    ...
    expect(clientCatalog).toMatchObject({ season: '2026/27', currentGameweek: 1 })
    ...
    const creatorPayload = (externalClaimId: string) => ({ source: { platform: 'YOUTUBE', externalId: 'video-1', creator: 'Fixture creator', title: 'GW1 notes', url: 'https://youtube.com/watch?v=fixture' }, claims: [{ externalClaimId, rawPlayerName: 'Mystery winger', category: 'ROLE', sentiment: 'POSITIVE', summary: 'Expected to start on the right.', startProbability: .72, confidence: .8 }] })
    const unmatchedResponse = await fetch(`${baseUrl}/api/signals/ingest`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: 'Bearer fixture-signal-token' }, body: JSON.stringify(creatorPayload('claim-unmatched')) })
    expect(unmatchedResponse.status).toBe(201)
    expect(await unmatchedResponse.json()).toMatchObject({ matched: 0, unresolved: 1 })
    const unmatchedClaims = await fetch(`${baseUrl}/api/creator-claims`).then(response => response.json())
    expect(unmatchedClaims.claims).toEqual([expect.objectContaining({ id: 'claim-unmatched', rawPlayerName: 'Mystery winger', matchStatus: 'UNRESOLVED' })])
    const resolveResponse = await fetch(`${baseUrl}/api/creator-claims/claim-unmatched`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ playerId: 10, rememberAlias: true }) })
    expect(resolveResponse.status).toBe(200)
    expect(await resolveResponse.json()).toMatchObject({ claimId: 'claim-unmatched', rememberedAlias: true, signal: { playerId: 10 } })
    const aliasMatchedResponse = await fetch(`${baseUrl}/api/signals/ingest`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: 'Bearer fixture-signal-token' }, body: JSON.stringify(creatorPayload('claim-alias-match')) })
    expect(aliasMatchedResponse.status).toBe(201)
    expect(await aliasMatchedResponse.json()).toMatchObject({ matched: 1, unresolved: 0, created: 1 })
