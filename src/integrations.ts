...
export type CanonicalRecommendationCandidate = { id: string; rank: number; action: string; apiMoves: Array<{ outId: number; inId: number }>; netExpectedGain: number; rawGain: number; hitCost: number; uncertaintyPenalty: number; savedTransferValue: number; lookaheadAvailable: boolean; nextWeekFreeTransfers: number | null; nextWeekBestNetGain: number | null; probabilityBeatsRoll: number | null; affordabilityStatus: string; bankAfterTenths: number | null; p10Points: number | null; p50Points: number | null; p90Points: number | null; leagueDifferential?: number | null; selectionGain?: number | null; selectionScoreVersion?: string; chip?: string; chipReason?: string; classification?: RecommendationClassification; timingAdvice?: string; priceTiming?: unknown; earlySeasonSensitive?: boolean; roleLatestMatchSensitive?: boolean; latestMatchSensitive?: boolean; latestMatchSensitivity?: 'LOW' | 'MEDIUM' | 'HIGH'; sensitivityFlags?: string[]; decision?: 'ACCEPTED' | 'REJECTED' | 'IGNORED' | 'CUSTOM' | null; recordedDecision?: { id: string; decision: string; createdAt: string } | null };
