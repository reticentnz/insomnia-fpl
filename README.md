...
For remote workflows, `/api/remote/signals` provides an authenticated JSON feed of the canonical signal ledger. Set `ADMIN_TOKEN` and send `Authorization: Bearer <ADMIN_TOKEN>`; the feed supports `status`, `playerId`, `since`, `actionableOnly=true`, and `limit` query parameters. Pending findings are marked `actionable` with a suggested `APPROVE` action. Apply a finding with `POST /api/remote/actions` and JSON such as `{"action":"approve","signalId":"...","reason":"Reviewed by automation"}` (or use `signalIds` for a batch). Approvals are audited as `REMOTE_API` actions and schedule a forecast recompute; this endpoint never changes the official FPL squad.
