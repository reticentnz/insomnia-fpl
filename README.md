...
For remote workflows, `/api/remote/signals` provides an authenticated JSON feed of the canonical signal ledger plus unresolved ambiguous player events from creator/RSS ingestion. Set `ADMIN_TOKEN` and send `Authorization: Bearer <ADMIN_TOKEN>`; the feed supports `status`, `playerId`, `since`, `actionableOnly=true`, and `limit` query parameters. Signal findings have `type: "SIGNAL"`; unresolved player events have `type: "PLAYER_EVENT"`, include candidate matches, and are marked with the suggested `RESOLVE_OR_DISMISS` action. `POST /api/remote/actions` supports `{"action":"reprocess"}` (or a `signalIds` subset): it downgrades unsupported or stale role interpretations to context-only, finalizes safe context, and leaves uncertain role claims pending. Add `"includeVerified":true` to audit existing verified model-impacting signals under the same gates; only those signals are selected, and unsupported/undated claims are returned to pending context for review. Apply a reviewed signal with `{"action":"approve","signalId":"...","reason":"Reviewed by automation"}` (or use `signalIds` for a batch). Resolve an ambiguous event with `{"action":"resolve_claim","claimId":"...","playerId":123,"rememberAlias":true}` or dismiss it with `{"action":"dismiss_claim","claimId":"..."}`. Approvals and resolutions are audited/recorded as remote actions and schedule a forecast recompute; this endpoint never changes the official FPL squad.
...
Run `npm run replay:fpl-core -- --gameweeks 1-5 --summary-only --output /tmp/fpl-core-replay.json` for an offline, archive-compatible walk-forward replay against the public FPL Core Insights 2025/26 archive. For each target gameweek the script selects the latest GitHub commit strictly before the deadline, pins player/team/fixture inputs to that commit, requires cumulative points and minutes to reconcile with all earlier gameweek outcomes, and whitelists only realized scoring outcomes from the later target file. It reports forecast error, component diagnostics, FPL baselines, top-K regret, a formation-constrained global XI and captain regret without writing synthetic rows to the production forecast or calibration tables. `eliteRanking` is a separate percentile selection score (60% calibrated mean rank, 30% no-bonus rank and 10% FPL PPG rank); it is used to preserve high-quality alternatives in candidate search and never replaces displayed xPts, calibration, hit economics or robust-primary thresholds. Omit `--summary-only` to retain player-level diagnostics. Cached source files live under `.cache/fpl-core-replay`; set `GITHUB_TOKEN` for a full-season run to avoid anonymous GitHub API limits. This is explicitly a core-model replay because historical market odds, underlying snapshots and the verified signal ledger are not present in the archive.
